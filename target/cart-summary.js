...
function runScriptWhenReady() {
    if (allElementsLoaded()) {
        ...
        detailsCards.forEach(card => {
            const itemName = card.querySelector('[data-test="cartItem-title"]')?.textContent.trim() || "";
            const quantity = card.querySelector('select')?.value || "1";
            const itemPrice = card.querySelector('[data-test="cartItem-price"]')?.textContent.trim() || "";
            let unitPrice = card.querySelector('[data-test="cartItem-unitPrice"]')?.textContent.trim() || "";
        ...
        const invoiceMetaElement = document.querySelector('[data-test="cart-item-groups"]');
        console.log(table)
